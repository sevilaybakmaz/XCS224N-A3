...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, t):
        ...
        x = x.view(x.size()[0], -1) 
        #N = t.shape[0]
        #x = torch.index_select(self.embeddings, 0, torch.flatten(t)).view(N, -1)
        ### END CODE HERE
        return x
    ...
    def forward(self, t):
        ...
        #embed = self.embedding_lookup(t) #(batch_size, n_features * embed_size)
        #hidden = F.relu(embed.mm(self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        #logits = hidden.mm(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
